    ...
    def __init__(self, config):
        ...
        if config.apply_lora and "query" in config.lora_module:
            if config.lora_type == "frd":
                ...
                self.query = lora.SVDLinear(config.hidden_size, self.all_head_size, r=config.lora_r, 
                                            lora_alpha=config.lora_alpha, merge_weights=False)
            else:
        ...
        if config.apply_lora and "key" in config.lora_module:
            if config.lora_type == "frd":
                self.key = lora.Linear(config.hidden_size, self.all_head_size, r=config.lora_r, 
                                                lora_alpha=config.lora_alpha, merge_weights=False)
            elif config.lora_type == "svd": 
                self.key = lora.SVDLinear(config.hidden_size, self.all_head_size, r=config.lora_r, 
                                            lora_alpha=config.lora_alpha, merge_weights=False)
            else:
                raise ValueError("Unimplemented Lora Type: %s"%config.lora_type)
        else:
        ...
        if config.apply_lora and "value" in config.lora_module:
            if config.lora_type == "frd":
                ...
                self.value = lora.SVDLinear(config.hidden_size, self.all_head_size, r=config.lora_r, 
                                            lora_alpha=config.lora_alpha, merge_weights=False)
            else:
                raise ValueError("Unimplemented Lora Type: %s"%config.lora_type)
        else:
